...
export default class WalletUtils {
  ...
  //       return data.tokens
  //         .filter(
  //           token => !availableTokensAddresses.includes(token.tokenInfo.address)
  //         )
  //         .forEach(token => {
  //           store.dispatch({
  //             type: ADD_TOKEN,
  ...
  //         value: (parseInt(t.value, 10) / Math.pow(10, 18)).toFixed(6),
  //         gasPrice: t.gasPrice,
  //         gasUsed: t.gasUsed
  //       }));
  //       store.dispatch({
  ...
  //         timestamp: t.timeStamp,
  //         transactionHash: t.hash,
  //         value: (parseInt(t.value, 10) / Math.pow(10, decimals)).toFixed(6),
  //         gasPrice: t.gasPrice,
  //         gasUsed: t.gasUsed
  //       }));
  //       store.dispatch({
  ...
  // ) {
  //   if (symbol === "ETH") {
  //     return this.sendETHTransaction(toAddress, amount, symbol);
  //   }
   ...
   * @param {String} amount
   */
  // static sendETHTransaction(toAddress, amount, symbol) {
  //   const web3 = this.getWeb3Instance();

  //   AnalyticsUtils.trackEvent("Send ETH transaction", {
  //     value: amount
  //   });
  //   const currentNetwork = store.getState().network;
