...
export const createRequestTypes = base => phases.reduce((acc, type) => {
...
export const createEntityApi = (entity, verbs = crudVerbs) => {
    ...
    apiMethods.forEach((verb) => {
        result.type[verb] = {};
        result.action[verb] = {};
        phases.forEach((phase) => { // redux types
            result.type[verb][phase] = `${entity}_${verb}_${phase}`;
        });
        phases.forEach((phase) => { // redux actions
            switch (phase) {
            case 'request':
        ...
        result.networkFetch[verb] = function* (params) { // saga for network fetch
        ...
        result.watchThunk[verb] = function* (params, watchAsync) { // wrapper for user api method
        ...
        result.afterWatch[verb] = function* () { // watcher for 'after'
            while (true) {
        ...
        result.failureWatch[verb] = function* () { // watcher for 'failure'
            while (true) {
    ...
    let watchVerbs = []
    _.forOwn(result.watch, function (value, key) {
        watchVerbs.push(value)
    })
    _.forOwn(result.afterWatch, function (value, key) {
        watchVerbs.push(value)
    })
    _.forOwn(result.failureWatch, function (value, key) {
        watchVerbs.push(value)
    })
    result.watchVerbs = function* (method) {
        for (var n = 0; n < watchVerbs.length; n++) {
            yield fork(watchVerbs[n])
        }
    }
    result.clearAfters = function() {
        _.forOwn(result.after, function (value, key) {
            _.unset(result.after, key)
        })
    }
    result.clearFailures = function() {
        _.forOwn(result.failure, function (value, key) {
...
function setFetchUrl(url, version) {
    ...
    if (!_.startsWith(fetchUrl, 'http') && _.has(__cfg, 'core.apiUrl')) {
        if (_.isUndefined(fetchUrl)) { // may be undefined if custom verb function is not setup by user
...
export function networkFetch(params = {}, files = []) {
    ...
    let apiUrl = '',
        config = setFetchConfig(params, files);
    ...
    return fetch(apiUrl, config)
        ...
        .catch(apiError => {
            return Promise.resolve(json(apiError)).then(error => {
                let response;
                if (_.isObject(error)) {
                    response = ({ error: {
                        ...error,
